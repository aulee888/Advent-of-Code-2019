...
def get_values(ip: int, r_base: int, prog: list, op: str, modes: str) -> list:
    ...
    if op in ['01', '02', '03' ,'04', '05', '06', '07', '08', '09']:
        ...
        if op in ['01', '02', '07', '08']:
            if modes[-3] == '0':
                values.append(prog[prog[ip + 3]])
            else:
                values.append(prog[r_base + prog[ip + 3]])
        '''
    return values
...
def run_comp(amp: Amp, phase: int, input: int) -> int:
    while amp.prog[amp.ip] != 99:
        ...
        #print(amp.prog[amp.ip + 3] > len(amp.prog))
        ...
        if op in ['01', '02', '07', '08']:
            if amp.prog[amp.ip + 3] > len(amp.prog):
                amp.prog = amp.prog + [0 for i in range(amp.prog[amp.ip + 3] - len(amp.prog) + 1)]
                #print('Current Length of Prog: {}'.format(len(amp.prog)))
                #print(amp.prog[1000])
        ...
        if op == '01':  # Add
            ...
            #if modes[-1] == '2':  # Relative Base input
            #    amp.prog[amp.r_base + amp.prog[amp.ip + 1]] = input
