...
'''
def values(prog, op, pos):
    if len(op) < 3:
        for amp.pos in range(3 - len(op)):
            op = '0' + op
...
def intcode(phase, input_val, amp):
    while True:
        operation = str(amp.prog[amp.pos])

        if operation[-2:] in ['01', '1']:  # Include both 01 and 1 to encompass both 1001 and 1 as opcodes
            #print('{} | {}, {}, {}, {}'.format(amp.pos, code[amp.pos], code[amp.pos+1], code[amp.pos+2], code[amp.pos+3]))
            temp = amp.prog[amp.prog[amp.pos + 1]]
            m, n = parameterize(operation, amp.prog, amp.pos)
            added_value = m + n
            amp.prog[amp.prog[amp.pos + 3]] = added_value
            #print('Target Index: {}, {} -> {}'.format(code[amp.pos+3], temp, code[code[amp.pos+3]]))
            amp.pos += 4

        elif operation[-2:] in ['02', '2']:  # Same as above
            #print('{} | {}, {}, {}, {}'.format(amp.pos, code[amp.pos], code[amp.pos+1], code[amp.pos+2], code[amp.pos+3]))
            temp = amp.prog[amp.prog[amp.pos + 3]]
            m, n = parameterize(operation, amp.prog, amp.pos)
            mult_value = m * n
            amp.prog[amp.prog[amp.pos + 3]] = mult_value
            #print('Target Index: {}, {} -> {}'.format(code[amp.pos+3], temp, code[code[amp.pos+3]]))
            amp.pos += 4

        # Only change to to this func is input_counter to account for when to use phase or amp
        elif operation == '3':  
            #print('{} | {}, {}'.format(amp.pos, code[amp.pos], code[amp.pos+1]))
            temp = amp.prog[amp.prog[amp.pos + 1]]
            if amp.input_counter == 0:
                amp.prog[amp.prog[amp.pos + 1]] = phase
                amp.input_counter += 1
            else:
                amp.prog[amp.prog[amp.pos + 1]] = input_val
            #print('Target Index: {}, {} -> {}'.format(code[amp.pos+1], temp, code[code[amp.pos+1]]))
            amp.pos += 2
        ...
        # 5 & 6 increase the instruction pointer (amp.pos) by 3 because only has three values in instruction amp.pos.e. (5, 1 ,2)
        elif operation[-2:] in ['05', '5']:
            #print('{} | {}, {}, {}'.format(amp.pos, code[amp.pos], code[amp.pos+1], code[amp.pos+2]))
            m, n = parameterize(operation, amp.prog, amp.pos)
            if m != 0:
                amp.pos = n
                #print('amp.pos : {}'.format(n))
            else:
                amp.pos += 3
            ...
            m, n = parameterize(operation, amp.prog, amp.pos)
            if m < n:
                amp.prog[amp.prog[amp.pos + 3]] = 1
            else:
                amp.prog[amp.prog[amp.pos + 3]] = 0
...
def signal_fb(code, test=None):
    ...
    if test == None:
        ...
        for phases in permutations(range(5,10), 5):
            amps = [Amp(code) for i in range(5)]
            active = 0

            for phase in phases:
                #print('Iteration: {}'.format(phase))
                #print('-'*15)
                output = intcode(code, phase, amps[active])
        ...
        while amps[4].halt != True:
            amps[active] = intcode(phases[active], amps[active - 1].output, amps[active])
            active = (active + 1) % 5 

        max_thrust = amps[4].output
...
# Answer to Part 2
print(signal_fb(code4, setting4))
